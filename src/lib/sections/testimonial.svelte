...
<div id="content-container" class="work-click-area" bind:this="{ workContainer }">
	<h1 style="margin-left: 20%;">Testimonials</h1>
					...
					<li use:workImageIntro={{ promise: inViewPromise, delay: i*30 }}>
						<div class="list-item clickable passive" 
							...
							<div class="img-wrapper">
								{#await loadImage(`${item.avatar}`) then src}
									<img bind:this={images[i]} src="{src}" on:dragstart|preventDefault draggable="false" alt="{item.name} Background">
								{/await}
							</div>
				...
				<div class="wrapper">
					<div class="top-align">
						<div class="wrapper">
							...
							<h6 class="caption">
								<div in:maskSlideIn={{ reverse: true }} out:maskSlideOut>{data[currentActive].company}</div>
							</h6>
						</div>
					</div>
